...
class Player {
  ...
  final String rank;
  final int gameYear;
  final int gameDay;  // 1–360 daily calendar
  ...
  factory Player.fromJson(Map<String, dynamic> j) => Player(
    ...
    // backward-compat: supports both game_day (new) and game_month (old) schemas
    gameDay: j['game_day'] != null
        ? (j['game_day'] as int)
        : ((j['game_month'] ?? 1) as int) * 30,
  );

  String get dateLabel =>
      'Year $gameYear  Day ${gameDay.toString().padLeft(3, '0')}';

  String get season {
    if (gameDay <= 90)  return 'Spring';
    if (gameDay <= 180) return 'Summer';
    if (gameDay <= 270) return 'Autumn';
    return 'Winter';
  }
}
...
class Ship {
  ...
  final String status;
  final int etaDays;   // days remaining
  final String? destination;

  Ship({
    required this.shipId, required this.name, required this.shipType,
    required this.cargoCap, required this.speedKnots,
    required this.currentCity, required this.status,
    required this.etaDays, this.destination,
  });
  ...
  factory Ship.fromJson(Map<String, dynamic> j) => Ship(
    ...
    speedKnots: double.tryParse(j['speed_knots']?.toString() ?? '5') ?? 5.0,
    currentCity: j['current_city'] ?? '',
    status: j['status'] ?? 'docked',
    // support both old eta_months and new eta_days
    etaDays: j['eta_days'] ?? ((j['eta_months'] ?? 0) * 30),
    destination: j['destination'],
  );

  /// Nominal speed label for UI
  String get speedLabel => '${speedKnots.toStringAsFixed(1)} kn';
...
class TradeLog {
  ...
  TradeLog({
    required this.gameYear, required this.gameDay, this.shipName, this.city,
  ...
  factory TradeLog.fromJson(Map<String, dynamic> j) => TradeLog(
    ...
    gameDay: j['game_day'] ?? ((j['game_month'] ?? 0) * 30),
...
class GameState extends ChangeNotifier {
  ...
  /// Format game date for display — 'Year 1304  Day 045  (Summer)'
  String fmtDay(int year, int day) {
    final season = day <= 90 ? 'Spring'
        : day <= 180 ? 'Summer'
        : day <= 270 ? 'Autumn' : 'Winter';
    return 'Year \$year  D\${day.toString().padLeft(3, \'0\')}  ·  \$season';
  }
  // backward-compat shim
  String fmtDate(int year, int dayOrMonth) => fmtDay(year, dayOrMonth);
  ...
  Future<bool> sailShip(ApiConfig api, int shipId, String destination, int etaDays) async {
    try {
      await http.patch(
          ...
          title: gs.player == null
              ...
              : Column(
                  ...
                  children: [
                    ...
                    Text('${gs.player!.rank}  ·  ${gs.fmtDay(gs.player!.gameYear, gs.player!.gameDay)}',
                      style: OTheme.textTheme.labelSmall),
                  ],
                ),
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: [
                ...
                _StatCell(label: 'SEASON', value: gs.player!.season),
                _StatCell(label: 'DAY',
                  value: gs.fmtDay(gs.player!.gameYear, gs.player!.gameDay)),
              ],
            ),
          ),
        ),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          _StatCell(label: 'SPEED', value: ship.speedLabel),
          const SizedBox(width: 24),
        ...
        Container(
          ...
          child: ElevatedButton(
            onPressed: _dest == null ? null : () async {
              ...
              // ETA from p3_routes.travel_days; fallback = distance_nm÷(speed*24)
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                  backgroundColor: ok ? OTheme.profit : OTheme.loss,
                  content: Text(ok
                    ? 'Setting sail for ${_dest!.name} — ETA $eta day(s)'
          ...
          (ctx, i) {
            ...
            return Container(
              ...
              child: Row(children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                  Text('Y${log.gameYear}·D${log.gameDay.toString().padLeft(3, \'0\')}',
                    style: OTheme.textTheme.labelSmall),
      ...
      SliverToBoxAdapter(
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            ..._endpointRows([
              ('p3_player', 'Gold, rank, game_year, game_day'),
              ...
              ('p3_price_history', 'Price history (every 10 days)'),
              ('p3_routes', 'Routes with travel_days'),
              ('p3_building_types', 'Buildings — daily production/maintenance'),
