...
p3_main_dashboard() {
    ...
    panel_left=$(
    {
        ...
        printf '%s\n' "$fleet_lines"
        printf '%s\n' "$ruler_l"
        printf '👁  Cities: %s  ·  Counting houses: %s\n' \
    ...
    if [[ "$is_admin" == "t" ]]; then
        ...
        panel_right=$(
        {
            printf '🏠  LÜBECK  |  pop:%-7s  |  %s  [hex %s]  |  counting houses: %s\n' \
        ...
        panel_right=$(
        {
            printf '🏠  LÜBECK  |  pop:%-7s  |  %s  |  region: %s  |  hex: %s  |  counting houses: %s\n' \
