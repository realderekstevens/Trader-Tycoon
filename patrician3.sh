...
#
#  Fresh-install version — no patch/migration code.
...
-- ── Hex grid (flat-top axial, 1 hex ≈ 50 nm) ─────────────────────────────
...
ALTER TABLE p3_cities
    ADD COLUMN IF NOT EXISTS hex_q INTEGER DEFAULT NULL,
    ADD COLUMN IF NOT EXISTS hex_r INTEGER DEFAULT NULL;
...
RETURNS TABLE(nq INT, nr INT) LANGUAGE sql IMMUTABLE AS $$
    VALUES (q+1,r),(q-1,r),(q,r+1),(q,r-1),(q+1,r-1),(q-1,r+1);
$$;

-- Travel days from hex distance: 1 hex = 50 nm, speed_kn × 24 = nm/day
CREATE OR REPLACE FUNCTION p3_travel_days(
    city_a_id INT, city_b_id INT, speed_kn NUMERIC DEFAULT 5.0)
RETURNS INTEGER LANGUAGE sql STABLE AS $$
    SELECT GREATEST(1,
        ROUND((p3_hex_distance(ca.hex_q,ca.hex_r,cb.hex_q,cb.hex_r) * 50.0)
              / (speed_kn * 24.0))::INTEGER)
...
SELECT
    ...
    CASE
        WHEN cg.good_id IS NOT NULL THEN LEAST(500, FLOOR(g.base_production * 450)::INTEGER)
...
-- name                      out_good      in_good   input_u   prod    cost   maint/day  notes
 ('Grain Farm',       'Grain',      NULL,         0.000, 0.233,  5000, 25.67, 'Raw — Hamburg/Gdansk/Stettin. Fills stock in ~90 days'),
 ...
 ('Brewery',          'Beer',       'Grain',      0.140, 1.633,  8000, 60.67, 'Grain→Beer — Hamburg/Gdansk/Bremen. Highest output building'),
...
SELECT g.good_id,
       CASE g.name
           WHEN 'Ivory'      THEN 0.75  -- rarest of all; steep price curve
           WHEN 'Silk'       THEN 0.70
...
# ── Hex city placement ────────────────────────────────────────────────────
p3_seed_hex_cities() {
    info "Placing cities on hex grid…"
    p3_psql <<'SQL'
-- Hanseatic Baltic (flat-top axial, 1 hex ≈ 50 nm)
WITH city_coords(city_name, q, r) AS (VALUES
    ('Lübeck',      14, 90), ('Hamburg',    14, 89), ('Rostock',    16, 90),
    ('Stettin',     21, 89), ('Gdansk',     25, 91), ('Riga',       31, 95),
    ('Reval',       30, 99), ('Novgorod',   40, 98), ('Stockholm',  21, 99),
    ('Visby',       22, 96), ('Malmö',      16, 93), ('Bergen',      1,101),
    ('Oslo',         9,100), ('Aalborg',    11, 95), ('Scarborough',  0, 90),
    ('Edinburgh',    2, 93), ('London',      1, 86), ('Brugge',      6, 85),
    ('Groningen',    9, 89), ('Bremen',     12, 89), ('Cologne',    11, 85),
    ('Torun',        3, 88), ('Ripen',      11, 92), ('Ladoga',     41,100),
    -- Mediterranean (offset to separate coordinate space)
    ('Venice',      110, 10), ('Genoa',      104, 12), ('Marseille',  100, 11),
    ('Barcelona',    97, 13), ('Lisbon',      92, 14), ('Constantinople',125,8),
    ('Naples',      111, 14), ('Palermo',    108, 16), ('Tunis',     107, 18),
    ('Alexandria',  118, 18)
)
UPDATE p3_cities ci
SET    hex_q = cc.q,
...
BEGIN
    ...
    LOOP
        IF o.action='buy' AND o.current_buy <= o.price_limit THEN
            ...
            IF fulfill > 0 THEN
                INSERT INTO p3_cargo (ship_id,good_id,quantity) VALUES (o.ship_id,o.good_id,fulfill)
                    ON CONFLICT (ship_id,good_id) DO UPDATE SET quantity=p3_cargo.quantity+fulfill;
                UPDATE p3_market SET stock=stock-fulfill WHERE city_id=o.city_id AND good_id=o.good_id;
                UPDATE p3_player SET gold=gold-(o.current_buy*fulfill);
                UPDATE p3_limit_orders SET remaining_quantity=remaining_quantity-fulfill WHERE order_id=o.order_id;
            END IF;
        ELSIF o.action='sell' AND o.current_sell >= o.price_limit THEN
            SELECT quantity INTO v_cargo FROM p3_cargo WHERE ship_id=o.ship_id AND good_id=o.good_id;
            fulfill := LEAST(o.remaining_quantity, COALESCE(v_cargo,0), 5);
            IF fulfill > 0 THEN
                UPDATE p3_cargo SET quantity=quantity-fulfill WHERE ship_id=o.ship_id AND good_id=o.good_id;
                DELETE FROM p3_cargo WHERE ship_id=o.ship_id AND good_id=o.good_id AND quantity<=0;
                UPDATE p3_market SET stock=stock+fulfill WHERE city_id=o.city_id AND good_id=o.good_id;
                UPDATE p3_player SET gold=gold+(o.current_sell*fulfill);
                UPDATE p3_limit_orders SET remaining_quantity=remaining_quantity-fulfill WHERE order_id=o.order_id;
            END IF;
        END IF;
        UPDATE p3_limit_orders SET active=FALSE WHERE order_id=o.order_id AND remaining_quantity<=0;
    END LOOP;
END $$;
SQL
}
...
p3_do_buy() {
    ...
    BEGIN
        ...
        INSERT INTO p3_trade_log
            (game_year,game_day,ship_id,ship_name,city,good_id,good_name,action,quantity,price,total_value,gold_after)
        SELECT pl.game_year, pl.game_day, $sid,
...
patrician_menu() {
    ...
    while true; do
        ...
        case "$choice" in
            "── Setup ──"|"── Fleet ──"|"── Trading ──"|"── Buildings ──"|"── Market ──"|"── Routes ──"|"── World ──"|"── Time ──"|"── Log ──"|"── Mediterranean ──")
                continue ;;
            ...
            "Reseed Market Prices")
                if confirm "Re-randomise all market prices (keeps stock levels)?"; then
                    p3_psql -c "
                        ...
                        FROM (
                            SELECT m2.city_id, m2.good_id,
                                   ROUND((g.buy_price_min+g.sell_price_min)/2.0
                                       *(0.85+(RANDOM()::NUMERIC)*0.30)
                                       *CASE WHEN cg.good_id IS NOT NULL THEN 0.88::NUMERIC ELSE 1.05::NUMERIC END,
                                   2) AS new_mid
                            FROM p3_market m2
                            JOIN p3_goods g ON g.good_id=m2.good_id
                            LEFT JOIN p3_city_goods cg ON cg.city_id=m2.city_id
                                AND cg.good_id=m2.good_id AND cg.role='produces'
            ...
            "Add Order to Route")
                ...
                if [[ "$action" == "buy" ]]; then
                    p3_psql -c "INSERT INTO p3_route_orders (route_id,city,good_id,action,quantity,max_price)
                                VALUES ($rid,'$city',$gid,'buy',$qty,$([ -n "$maxp" ] && echo "'$maxp'" || echo 'NULL'));" >/dev/null
                else
                    p3_psql -c "INSERT INTO p3_route_orders (route_id,city,good_id,action,quantity,min_price)
                                VALUES ($rid,'$city',$gid,'sell',$qty,$([ -n "$maxp" ] && echo "'$maxp'" || echo 'NULL'));" >/dev/null
                fi
            ...
            "View All Cities")
                p3_psql -c "
                    SELECT name, region, league, population,
                           COALESCE(hex_q::text,'?')||','||COALESCE(hex_r::text,'?') AS hex
            ...
            "Cross-League Opportunities  (Hanse ↔ Med)")
                p3_psql -c "
                    ...
                    LEFT JOIN p3_routes r ON
                        ...
                        (r.city_b=av.buy_city AND r.city_a=av.sell_city)
                    WHERE av.profit_per_unit > 0
                      AND ((av.buy_city  IN (SELECT name FROM p3_cities WHERE league='Hanseatic')
                            AND av.sell_city IN (SELECT name FROM p3_cities WHERE league='Mediterranean'))
                           OR
                           (av.buy_city  IN (SELECT name FROM p3_cities WHERE league='Mediterranean')
                            AND av.sell_city IN (SELECT name FROM p3_cities WHERE league='Hanseatic')))
        ...
        case "$choice" in
            ...
            "Expand Existing Building")
                ...
                if confirm "Expand by $add_n for ${total_cost}g?"; then
                    p3_psql -c "
                        UPDATE p3_player_buildings SET num_buildings=num_buildings+$add_n WHERE pb_id=$pb_id;
            ...
            "Cancel Limit Order")
                ...
                oid=$(p3_psql --tuples-only -c "
                    SELECT lo.order_id||' – '||lo.action||' '||lo.remaining_quantity||' '||g.name
            ...
            "Preview Marginal Price Curve")
                ...
                p3_psql -c "
                    SELECT
                        ...
                        ROUND(
                            ((m.current_buy/1.08 + m.current_sell/0.92)/2.0)
                            * POWER(e.stock_ref::NUMERIC / GREATEST(m.stock - generate_series(1,20) + 1, 1)::NUMERIC,
                                    e.elasticity_buy)
                            * 1.08,
                        2) AS marginal_ask
                    FROM p3_market m
